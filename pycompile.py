...
def CompileSharedLibrary(exname, src_dir,obj_dir,filelist, libraries, dbg, opt, cppv=CPPOX, libpath=[], incpath=[]):
    ...
    if(libraries != ""):
        link_args += " " + libraries
    link_call = "g++ -shared {0} -O{1} -g{2} {4} -std=c++0x -o {3}".format(link_args,opt,dbg,exname,link_inc)
    print link_call
    do_call(link_call)
...
def Compile(exname, src_dir,obj_dir,filelist, libraries, dbg, opt, cppv=CPPOX, libpath=[], incpath=[]):
    ...
    for i in filelist:
        ...
        print ">",i_stripped
        if i not in pycache.keys() or pycache[i] < chtime:
            call = ("{0} -c {1} {2}-O{3} -g{4} -std=c++0x {6} -o {5}").format("g++", code_path,includes,opt,dbg,object_path,incpath)
            rmcall ="rm " + object_path
