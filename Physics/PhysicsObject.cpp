...
#include <iostream>
	...
	bool PhysicsObject::LoadPolygon(Polygon P){
		if(P.Vertex.size()==0)
			return false;
	...
	bool PhysicsObject::LoadPolygon(double * vertex_list,unsigned int n_vertex){
		Polygon temp_poly;
		for(unsigned int i=0;i<n_vertex;i++){
			temp_poly.AddVertex(vertex_list[i*2],vertex_list[i*2+1]);
		}
		return LoadPolygon(temp_poly);
	}
	...
	void PhysicsObject::CalculateMomentofInertia(){
		double TotalArea=0;
		for(std::list<Polygon>::iterator it=Body.begin();it!=Body.end();it++){
	...
	void PhysicsObject::FindBounds(){
		for(std::list<Polygon>::iterator itf=Body.begin();itf!=Body.end();itf++){
			itf->FindBounds();
		}
	}

	void PhysicsObject::UpdateGeometricData(){
		for(std::list<Polygon>::iterator itf=Body.begin();itf!=Body.end();itf++){
			itf->FindBounds();
			itf->CalculateAxisAndEdges();
		}
	}
